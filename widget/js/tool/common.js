	...
	$.ajax({
		...
		error:function(XMLHttpRequest, textStatus, errorThrown,fnErr){
            ...
            if(XMLHttpRequest.status=='404') {
                var htmlStr =   '<div class="warp">'+
                                    '<div class="page">'+
                                        '<img src="./images/commPage/404.png" alt=""style="width:88px;height:92px;">'+
                                        '<span style="color:#999">找不到服务器</span>'+
                                    '</div>'+
                                '</div>';
                $('#mainStage').html(htmlStr);
                return;
            }
            ...
            if(XMLHttpRequest.status=='500') {
                var htmlStr =   '<div class="warp">'+
                                    '<div class="page">'+
                                        '<img src="./images/commPage/500.png" alt="" style="width:168px;height:168px;">'+
                                        '<span>主人，服务器出差错了 </span>'+
                                    '</div>'+
                                '</div>';
                $('#mainStage').html(htmlStr);
                return;
...
var IsMobile = function (text){
    ...
    var _z = /^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/;
    if (_d.test(text) || _l.test(text) || _y.test(text) || _z.test(text)) {
        return 1;
    }
    return 0;
};
...
$("html,body").animate({scrollTop:0}, 500);
 ...
 //        domain_url = domain_url.split('/')[0];
 //    }
 //    document.cookie = "JSESSIONID="+dataGet('user_token')+";path=/;domain="+domain_url;
 //};
 // UrlToken();
...
$.ajaxSettings.beforeSend = function(xhr,request){
    ...
    // 在这里加上你的 token
    xhr.setRequestHeader('token',dataGet('user_token'));
}